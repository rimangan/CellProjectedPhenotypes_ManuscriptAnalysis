...
# -- tidy 
deseq_to_df_volcano_unsafe <- function(res) {
  df <- as.data.frame(res)
  gene <- if ("geneName" %in% names(df)) df$geneName else rownames(df)
  # choose FDR source
  pval <- suppressWarnings(as.numeric(df$pvalue))
...
p_sg <- ggplot(sg_plot_data_simple, aes(x = celltype, y = signed_count, fill = direction)) +
  geom_col() +
  geom_text(
    data = sg_plot_data_simple %>% filter(direction == "Down" & count > 0),
    aes(label = count, y = signed_count),
    hjust = 1.1, size = 3, color = "black"
  ) +
  geom_text(
    data = sg_plot_data_simple %>% filter(direction == "Up" & count > 0),
    aes(label = count, y = signed_count),
    hjust = -0.1, size = 3, color = "black"
  ) +
  coord_flip() +
  scale_y_continuous(limits = c(-sg_lim, sg_lim)) +
  scale_fill_manual(values = direction_colors, name = "Direction") +
  theme_classic() +
...
p_cc <- ggplot(cc_plot_data, aes(x = celltype, y = signed_count, fill = direction)) +
  geom_col() +
  geom_text(
    data = cc_plot_data %>% filter(direction == "Down" & count > 0),
    aes(label = count, y = signed_count),
    hjust = 1.1, size = 3, color = "black"
  ) +
  geom_text(
    data = cc_plot_data %>% filter(direction == "Up" & count > 0),
    aes(label = count, y = signed_count),
    hjust = -0.1, size = 3, color = "black"
  ) +
  coord_flip() +
  scale_y_continuous(limits = c(-cc_lim, cc_lim)) +
  scale_fill_manual(values = cc_direction_colors, name = "Direction") +
  theme_classic() +
