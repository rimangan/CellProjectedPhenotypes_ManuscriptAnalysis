...
merged_df_final <- readRDS("Data/merged_df_final_12_18.rds")
combined_metadata <- combined_metadata |>
  dplyr::left_join(
    dplyr::select(merged_df_final, projid, batch),
    by = "projid"
  )
...
run_one <- function(A, B, combined_metadata) {
  ...
  sanitize_meta <- function(cm) {
    ...
    need <- c("projid", "pmi", "msex", "age_death", "batch")
  ...
  dds <- DESeq2::DESeqDataSetFromMatrix(
    countData = counts,
    colData   = colData,
    design    = ~ pmi + msex + age_death + batch + group
  )
...
if (length(all_res)) {
  ...
  write_tsv(combo, "Tables/DataS3.SuperneighborhoodDegCompleteResults.tsv.gz")
  write_tsv(combo_sig, "Tables/TableS6.SuperneighborhoodSignificantDEGs.tsv")
}
...
make_volcano <- function(df,
  ...
  # significance direction (FDR AND log2FC threshold)
  d$sig_dir <- "ns"
  d$sig_dir[d$FDR < fdr_thr & d$log2FC > lfc_thr] <- "pos_sig"
  d$sig_dir[d$FDR < fdr_thr & d$log2FC < -lfc_thr] <- "neg_sig"
  ...
  p <- ggplot2::ggplot(d, ggplot2::aes(x = log2FC, y = mlog10FDR)) +
    ...
    ggplot2::geom_vline(xintercept = c(-lfc_thr, lfc_thr), linetype = "dashed", linewidth = 0.5, color = "grey40") +
  ...
  message(sprintf("[volcano] %s — FDR<%.2f & |log2FC|>%.1f: up=%d, down=%d; labeled up=%d, down=%d",
...
p1 <- make_volcano(
  ...
  label_per_side = 20, fdr_fill2 = 0.20
)
...
readr::write_tsv(
  fgsea_tbl %>%
    select(-leadingEdge, -pathway_display, -gs_id),
  "Tables/TableS7.SuperneighborhoodFgseaResults.tsv.gz"
)
...
pdf("pdf/fgsea.plots.pdf", height=8, width=10)
p
dev.off()
...
micResults <- readRDS("Data/DEGs/noApoeCovariate/Mic P2RY12.rds")
snResults <- all_res
mic_df <- as.data.frame(micResults$subgroups) %>%
  select(gene = geneName, log2FoldChange_Mic = log2FoldChange)


sn_dfs <- lapply(names(snResults), function(name) {
  sn <- as.data.frame(snResults[[name]])
  sn <- sn %>%
    select(gene, !!paste0("log2FoldChange_", name) := log2FoldChange)
})

sn_merged <- Reduce(function(x, y) full_join(x, y, by = "gene"), sn_dfs)
mergedResults <- full_join(sn_merged, mic_df, by = "gene")


plot_fc_comparison <- function(mergedResults, col1, col2, myColor = "grey60") {
  p <- ggscatter(
    mergedResults,
    col1,
    col2,
    ...
    stat_cor(method = "pearson", label.x.npc = "left", label.y.npc = "top", size = 3.5)  # shows R and p
  return(p)
}

plot_fc_comparison <- function(mergedResults, col1, col2) {
  ggplot(mergedResults, aes_string(x = col1, y = col2)) +
    geom_bin2d(bins = 50) +  # adjusts density resolution
    ...
    theme(
      legend.position = "none",
      panel.border = element_rect(color = "gray60", fill = NA, linewidth = 0.4),
      axis.line = element_line(color = "gray40")
    )
}


a <- plot_fc_comparison(mergedResults, "log2FoldChange_Mic", "log2FoldChange_AD_SN_5_vs_HC_SN_8")#, myColor="#c3932c")
b <- plot_fc_comparison(mergedResults, "log2FoldChange_Mic", "log2FoldChange_AD_SN_2_vs_HC_SN_7")#, myColor="#f78e2c")
c <- plot_fc_comparison(mergedResults, "log2FoldChange_Mic", "log2FoldChange_AD_SN_3_vs_HC_SN_9")#, myColor="#9c2327")
d <- plot_fc_comparison(mergedResults, "log2FoldChange_AD_SN_2_vs_HC_SN_7", "log2FoldChange_AD_SN_5_vs_HC_SN_8")
e <- plot_fc_comparison(mergedResults, "log2FoldChange_AD_SN_2_vs_HC_SN_7", "log2FoldChange_AD_SN_3_vs_HC_SN_9")
f <- plot_fc_comparison(mergedResults, "log2FoldChange_AD_SN_5_vs_HC_SN_8", "log2FoldChange_AD_SN_3_vs_HC_SN_9")
pdf("pdf/effectSizeComparisons.pdf", height=6, width=4)
grid.arrange(a,d,b,e,c,f)
dev.off()
